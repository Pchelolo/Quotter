...
//Rewrite this with normal binary search and so on
object StemmedResolver {
	val dictionary  = (for(word <- Source.fromFile("rusnoun.txt").getLines) yield word).toSeq
	
	def getFullNoun(stemmed : String) = {
	  val iter = dictionary.iterator
	  var result : String = "not found"
	    do {
	      result = iter.next
	    }
	    while(iter.hasNext && !result.startsWith(stemmed) )
	    result
	}
	def getNouns(text : String) : Seq[String] = for((word, partOfSpeech) <- StemmerPorter.stemText(text) if partOfSpeech == PartOfSpeech.Noun) yield getFullNoun(word)
}
}